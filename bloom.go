...
// client is instance of redis.UniversalClient
...
func New(client redis.UniversalClient, name string, n uint64, p float64) (bloom *Bloom, err error) {
	info, err := client.HGetAll(ctx, name).Result()
	if err == nil {
		if len(info) != 0 {
...
func GetByName(client redis.UniversalClient, name string) (bloom *Bloom, err error) {
	info, err := client.HGetAll(ctx, name).Result()
	// fmt.Println(info)
	if err != nil {
		return nil, err
...
func (b *Bloom) Add(value []byte) error {
	...
	for _, loc := range locs {
		pipe.SetBit(ctx, loc.Name, int64(loc.Offset), 1)
	}
	_, err := pipe.Exec(ctx)
	return err
}
...
func (b *Bloom) Exists(value []byte) (bool, error) {
	...
	for i, loc := range locs {
		cmd := pipe.GetBit(ctx, loc.Name, int64(loc.Offset))
		res[i] = cmd
	}
	_, err := pipe.Exec(ctx)
	if err != nil {
...
func (b *Bloom) initStorage() (err error) {
	...
	for _, p := range parts {
		_, err = client.SetBit(ctx, p.Name, int64(p.Max), 0).Result()
		if err != nil {
			panic(err)
		}
	}
...
func (b *Bloom) saveParams() (err error) {
	...
	_, err = b.client.HMSet(ctx, b.name, info).Result()
	return err
}
